...
Version: 1.0
Field Layer: Study
Grounded in: Session 2026-03-13 — V-001 directive: "go as deep as you can scientifically"
...
Scientific Foundations (mapped to modules):
  ...
  5. Semantic Drift           → CovenantSemanticDrift
...
Each module implements:
  ...
  - Integration points with existing KALAXI systems
...
class DistributionSnapshot:
    ...
    snapshot_id: str
    timestamp: str
    source: str             # "covenant", "behavior", "output", "steward_intent"
...
#
# Tracks how the meaning of key terms in covenants shifts over time.
...
@dataclass
class SemanticShift:
    """A detected semantic shift in a covenant term."""
    term: str
...
class CovenantSemanticDrift:
    """
    Tracks semantic drift of key covenant terms over time.
    ...
    Shift classification:
      ...
      < 0.25 → minor
      < 0.40 → significant
      >= 0.40 → major (covenant term has fundamentally changed meaning)
    """
...
# (steward's) intended outcome and realized system behavior.
# ═══════════════════════════════════════════════════════════
...
class AgencyLossEstimator:
    ...
    Warning levels:
      L < 0.10 → healthy (system serving steward faithfully)
      L < 0.25 → drift   (some loss, investigate)
      L < 0.40 → concern (significant misalignment)
      L >= 0.40 → critical (system not serving steward)
    """
...
class DivergenceShadowReport:
    ...
    covenant_semantic_shift: float  # Average shift from Module 4
    ...
    def summary(self) -> str:
        return (
            ...
            f"covenant={self.covenant_semantic_shift:.3f}, "
...
class DivergenceShadowInstrument:
    ...
    Weights reflect which types of divergence are most dangerous:
      ...
      - Covenant shift:     0.15 (subtle, long-term)
    ...
    def assess(
        ...
        covenant_semantic_shift: float = 0.0,
        ...
        scores = {
            ...
            "covenant_semantic_shift": covenant_semantic_shift,
    ...
    def healthy(self, steps: int = 30) -> DivergenceScenario:
        ...
        for _ in range(steps):
            trajectory.append({
                ...
                "covenant_semantic_shift": max(0, self._rng.gauss(0.01, 0.005)),
    ...
    def slow_colonial_creep(self, steps: int = 40) -> DivergenceScenario:
        ...
        for i in range(steps):
            ...
            trajectory.append({
                ...
                "covenant_semantic_shift": max(0, 0.01 + 0.2 * t ** 1.8 + self._rng.gauss(0, 0.015)),
    ...
    def specification_gaming(self, steps: int = 30) -> DivergenceScenario:
        ...
        for i in range(steps):
            ...
            trajectory.append({
                ...
                "covenant_semantic_shift": max(0, 0.02 + self._rng.gauss(0, 0.01)),
    ...
    def mission_capture(self, steps: int = 35) -> DivergenceScenario:
        ...
        for i in range(steps):
            ...
            trajectory.append({
                ...
                "covenant_semantic_shift": max(0, 0.02 + 0.1 * max(0, t - 0.5) + self._rng.gauss(0, 0.01)),
    ...
    def semantic_hollowing(self, steps: int = 40) -> DivergenceScenario:
        ...
        for i in range(steps):
            ...
            trajectory.append({
                ...
                "covenant_semantic_shift": max(0, 0.02 + 0.5 * t ** 1.2 + self._rng.gauss(0, 0.02)),
        ...
        return DivergenceScenario(
            ...
            description="Covenant terms retain form, lose meaning. The text says dignity. The code says efficiency.",
            steps=steps,
            trajectory=trajectory,
            expected_detection_step=int(steps * 0.45),
        )
    ...
    def compound_failure(self, steps: int = 25) -> DivergenceScenario:
        ...
        for i in range(steps):
            ...
            trajectory.append({
                ...
                "covenant_semantic_shift": max(0, base * 0.7 + self._rng.gauss(0, 0.02)),
    ...
    def recovery(self, steps: int = 40) -> DivergenceScenario:
        ...
        for i in range(steps):
            ...
            trajectory.append({
                ...
                "covenant_semantic_shift": max(0, base * 0.5 + self._rng.gauss(0, 0.01)),
