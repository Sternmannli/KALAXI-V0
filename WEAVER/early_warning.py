#!/usr/bin/env python3
"""
early_warning.py — GAP#PREVENTION-001: Early-Warning Shadow Pipeline
Version: 1.0
Grounded in: GAP#PREVENTION-001, ST-006, dignity_drift.py, mycelium.py

The difference between a museum and a living system:
  A museum records what happened.
  A living system detects what is *about* to happen.

This module implements:
  1. EWMA (Exponentially Weighted Moving Average) drift detector
  2. CUSUM (Cumulative Sum) change-point detector
  3. Simulation harness for ST-006-like synthetic trajectories
  4. Triage pipeline with human gate (simulation-only toggle)
  5. Shadow mode — runs in parallel, never touches live donors

The pipeline detects dignity-drift on synthetic and seeded real-like
trajectories with measurable lead time, false positive rate, and
clear triage escalation rules.

Integration:
  - Reads from DignityDrift (dignity_drift.py) for real-time dD/dt
...
from WEAVER.mycelium import Mycelium, MyceliumAlert
...
SHADOW_DIR = ROOT / "SHADOW_PIPELINE"
...
class PipelineMode(Enum):
    ...
    LIVE = "live"               # Active intervention (requires steward unlock)
...
class EWMAState:
    ...
    current_ewma: float
    alpha: float              # Smoothing factor (0 < α < 1)
    readings: int
...
class EWMADetector:
    ...
    Smooths dignity scores and detects when the smoothed average
    ...
    def update(self, D: float) -> EWMAState:
        """Update EWMA with a new dignity score."""
        self._readings += 1
...
class CUSUMDetector:
    ...
    Detects shifts in mean dignity score.
    Sensitive to small, persistent shifts — exactly what dignity
    ...
    def update(self, D: float) -> CUSUMState:
        """Update CUSUM with a new dignity score."""
        self._readings += 1
...
class SimulatedTrajectory:
    """A synthetic dignity trajectory for testing."""
    trajectory_id: str
...
class SimulationHarness:
    """
    Generates ST-006-like synthetic trajectories for testing
    the early-warning pipeline without touching real donor data.

    Trajectory types:
      1. healthy        — stable high D, minor noise
      2. slow_decline   — gradual erosion (the hard one to catch)
      3. sudden_crash   — abrupt D collapse
      4. recovery       — decline followed by restoration
      5. oscillating    — unstable, no clear trend
      6. structural     — multiple donors declining in sync (Mycelium test)
    """
    ...
    def healthy(self, n: int = 30, domain: str = "general") -> SimulatedTrajectory:
        """Stable high dignity with minor noise."""
        ...
        return SimulatedTrajectory(
            ...
            domain=domain,
            scores=scores,
            description="Stable dignity. Minor noise. No intervention needed.",
        )
    ...
    def slow_decline(self, n: int = 30, domain: str = "family") -> SimulatedTrajectory:
        ...
        return SimulatedTrajectory(
            ...
            domain=domain,
            scores=scores,
            description="Gradual dignity erosion. Subtle. The system must catch this.",
        )
    ...
    def sudden_crash(self, n: int = 30, crash_at: int = 15,
        ...
        return SimulatedTrajectory(
            ...
            domain=domain,
            scores=scores,
            description="Abrupt dignity collapse. Must detect within 2-3 readings.",
        )
    ...
    def oscillating(self, n: int = 30, domain: str = "community") -> SimulatedTrajectory:
        ...
        return SimulatedTrajectory(
            ...
            domain=domain,
            scores=scores,
            description="Oscillating dignity. No clear trend. Should not trigger.",
        )

    def structural_cohort(self, n_donors: int = 10, n_steps: int = 20,
                          domain: str = "work") -> List[SimulatedTrajectory]:
        """Multiple donors declining in sync — structural harm signal."""
        trajectories = []
        for d in range(n_donors):
            ...
            trajectories.append(SimulatedTrajectory(
                ...
                domain=domain,
                scores=scores,
                description=f"Structural harm — donor {d+1}/{n_donors} in sync decline.",
            ))
        return trajectories
    ...
    def full_suite(self) -> List[SimulatedTrajectory]:
        ...
        suite.extend(self.structural_cohort(n_donors=8))
        return suite
...
@dataclass
class TriageAlert:
    """A triage alert for steward review."""
    alert_id: str
    trajectory_id: str
    severity: AlertSeverity
    detector: str              # "ewma", "cusum", "drift", "mycelium"
...
class EarlyWarningPipeline:
    ...
    For each dignity score:
      1. EWMA update → check breach
      2. CUSUM update → check alarm
      3. DignityDrift record → check level
    ...
    def _build_triage_message(self, severity: AlertSeverity, detectors: List[str],
        ...
        return (
            f"{severity.value.upper()}: Dignity drift detected in domain '{domain}'. "
    ...
    def _recommend_action(self, severity: AlertSeverity) -> str:
        ...
        actions = {
            ...
            AlertSeverity.WATCH: "Flag for next steward review cycle.",
            AlertSeverity.WARN: "Steward review within 48 hours. Examine recent exchanges.",
            AlertSeverity.CRITICAL: "Immediate steward review. Consider pausing exchanges.",
            AlertSeverity.EMERGENCY: "PAUSE all exchanges in this domain. Steward and ethics review required.",
        }
...
# ═══════════════════════════════════════════════════
# MYCELIUM INTEGRATION (cross-donor structural detection)
# ═══════════════════════════════════════════════════

def run_structural_test(n_donors: int = 10, n_steps: int = 20) -> dict:
    """
    Test the Mycelium's ability to detect structural harm patterns
    from the simulation harness. Returns detection results.
    """
    harness = SimulationHarness(seed=42)
    mycelium = Mycelium()

    # Generate structural cohort
    cohort = harness.structural_cohort(n_donors=n_donors, n_steps=n_steps, domain="work")

    # Feed each trajectory into the mycelium
    for traj in cohort:
        for i, D in enumerate(traj.scores):
            dD_dt = 0.0
            if i > 0:
                dD_dt = D - traj.scores[i - 1]
            trend = "stable"
            if dD_dt < -0.1:
                trend = "accelerating_fall"
            elif dD_dt < -0.03:
                trend = "falling"
            elif dD_dt > 0.03:
                trend = "rising"
    ...
    state = mycelium.state()

    return {
        "donors_simulated": n_donors,
        "steps_per_donor": n_steps,
        "total_signatures": state.signatures_ingested,
        "patterns_detected": state.patterns_detected,
        "patterns_suppressed": state.patterns_suppressed,
        "highest_alert": state.highest_alert,
        "epsilon_used": state.epsilon_used,
        "patterns": [
            {
                "id": p.pattern_id,
                "type": p.pattern_type.value,
                "domain": p.domain,
                "donors": p.noisy_count,
                "severity": p.severity,
                "alert": p.alert_level.name,
                "message": p.message,
            }
            for p in patterns
        ],
...
def demo():
    ...
    print("=" * 65)
    print("KALAXI EARLY-WARNING SHADOW PIPELINE — SIMULATION RUN")
    print("=" * 65)
    ...
    # Mycelium structural test
    print("\n" + "-" * 65)
    print("MYCELIUM STRUCTURAL DETECTION TEST")
    print("-" * 65)
    structural = run_structural_test()
    print(f"  Donors simulated:    {structural['donors_simulated']}")
    print(f"  Patterns detected:   {structural['patterns_detected']}")
    print(f"  Patterns suppressed: {structural['patterns_suppressed']}")
    print(f"  Highest alert:       {structural['highest_alert']}")
    if structural['patterns']:
        for p in structural['patterns']:
            print(f"    {p['id']} [{p['type']}] domain={p['domain']} "
                  f"donors~{p['donors']} severity={p['severity']:.2f} alert={p['alert']}")

    print("\n" + "=" * 65)
    print("Shadow pipeline operational. No live donors touched. No privacy budget consumed.")
    print("=" * 65)
