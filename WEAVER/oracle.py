...
Version: 1.0
Grounded in: GAP#019, FOUNDATIONS/witness_scale.md, FOUNDATIONS/proprioception_axiom.md
...
class WitnessLevel(Enum):
    """Six levels measuring the relationship between system and steward."""
    UNSEEN = 0      # W-0: Exists in registry, never referenced
    PASSED = 1      # W-1: System processed it, no steward contact
    FLAGGED = 2     # W-2: System surfaced it, steward has not responded
    SEEN = 3        # W-3: Steward has acknowledged it
    HELD = 4        # W-4: Steward has returned to it across sessions
    EMBODIED = 5    # W-5: Steward used it to make a system-changing decision
...
class WitnessRecord:
    ...
    element_id: str
    element_type: str       # "proverb", "anomaly", "covenant", "gap", "wisdom"
    level: WitnessLevel
...
class WitnessScale:
    ...
    the system and its steward around that content.

    Rules:
      1. W-Scale is non-decreasing — once W-3, cannot fall below W-3
      2. Transitions are append-only (COV#004)
    ...
    def transition(self, element_id: str, to_level: WitnessLevel,
        ...
        # Log transition (append-only, COV#004)
    ...
    def flag(self, element_id: str, context: str = "") -> Optional[WitnessRecord]:
        """Surface an element for steward attention (→ W-2)."""
    ...
    def steward_sees(self, element_id: str, session_id: str = "") -> Optional[WitnessRecord]:
        """Steward acknowledges an element (→ W-3)."""
    ...
    def steward_holds(self, element_id: str, session_id: str = "") -> Optional[WitnessRecord]:
        """Steward returns to an element across sessions (→ W-4)."""
    ...
    def steward_embodies(self, element_id: str, session_id: str = "",
        ...
        """Steward uses element to change the system (→ W-5)."""
    ...
    def witnessed(self) -> List[WitnessRecord]:
        """All elements at W-3+ (steward has witnessed)."""
        return [r for r in self._elements.values()
...
class Oracle:
    """
    The Oracle — self-audit layer for the KALAXI system.
    ...
    def audit(
        ...
        if creep_risk >= 0.5:
            findings.append(AuditFinding(
                ...
                "Invisible drift between covenant text and system behavior detected",
            ))
        elif creep_risk >= 0.3:
            findings.append(AuditFinding(
                "colonial_creep", AuditSeverity.ADVISORY,
                f"Colonial Creep risk moderate: {creep_risk:.0%}",
                "Monitor for covenant-behavior divergence",
            ))
