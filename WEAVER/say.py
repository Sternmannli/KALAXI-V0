...
# Somatic vocabulary — the body of AXI's voice (from AXI_VOICE_CANON.md)
SOMATIC_ANCHORS = {
    # Body
...
def audit_voice(content, context="") -> VoiceAudit:
    ...
    if not (SOMATIC_ANCHORS & words_set):
        violations.append(VoiceViolation(
            ...
            description="No somatic anchor — AXI requires concrete nouns (stone, hand, breath, knot...)",
            severity="warn",
        ))
    ...
    for phrase in HELPFULNESS_LEAK:
        if phrase in content_lower:
            violations.append(VoiceViolation(
                ...
                description=f"Helpfulness leak detected: '{phrase}' — AXI witnesses, not assists",
                severity="warn",
            ))
            break
