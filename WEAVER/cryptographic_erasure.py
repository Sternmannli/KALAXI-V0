#!/usr/bin/env python3
"""
cryptographic_erasure.py — Cryptographic Erasure Module v1.0
Resolves GAP#LEGAL-ERASURE-001: GDPR Article 17 vs COV#003 append-only.

The tension:
  GDPR Article 17 grants donors the "right to erasure" — personal data must
  be deletable upon request. But COV#003 mandates append-only storage: nothing
  is silently discarded, every operation leaves a trace.

The resolution — encrypt-then-delete-key:
  All donor data is encrypted before storage using a per-donor key derived
  from a master secret + donor_id. The ciphertext is written to the append-only
  ledger (COV#003 satisfied). To "erase," the donor's key material is destroyed.
  The ciphertext remains in the ledger but is cryptographically unrecoverable
  (GDPR Article 17 satisfied). The erasure event itself is recorded as an
  append-only audit entry with timestamp.
...
Cryptographic design (stdlib only, zero external dependencies):
  ...
  - Each donor gets a unique 256-bit key: HKDF(master_key, donor_id)
  - Nonce: 12 bytes from os.urandom per seal operation (never reused)

Covenant obligations:
  COV#001 — Donor identity protected. Encryption at rest.
  COV#003 — Privacy by default. Append-only ledger preserved.
...
ERASURE_DIR = ROOT / "MANIFEST" / "erasure"
...
def _derive_donor_key(master_key: bytes, donor_id: str) -> bytes:
    """
    Derive a unique 256-bit key for a donor.

    Uses HKDF (RFC 5869) with:
      - IKM: master_key
      - salt: SHA-256("kalaxi-erasure-salt-v1")
      - info: "donor-key:" + donor_id encoded as UTF-8
    """
    salt = hashlib.sha256(b"kalaxi-erasure-salt-v1").digest()
    prk = _hkdf_extract(salt, master_key)
    info = f"donor-key:{donor_id}".encode("utf-8")
...
class CryptographicErasure:
    """
    Cryptographic erasure engine for KALAXI donor data.

    Resolves GAP#LEGAL-ERASURE-001: GDPR Article 17 requires the right to
    erasure. COV#003 requires append-only storage. This engine encrypts data
    before storage (per-donor key) and "erases" by destroying the key.
    Ciphertext persists in the ledger (append-only) but is cryptographically
    unrecoverable (erasure satisfied).

    Usage:
        engine = CryptographicErasure(master_key=os.urandom(32))
        blob = engine.seal("DONOR-001", b"sensitive donor data")
        data = engine.unseal("DONOR-001", blob)   # returns plaintext
        engine.erase("DONOR-001")                   # destroys key
        data = engine.unseal("DONOR-001", blob)   # returns None — erased

    Covenant compliance:
        COV#001 — Encryption at rest protects identity.
        COV#003 — Ciphertext remains in append-only ledger.
        COV#010 — Erasure events recorded, never silent.
        COV#015 — Donor controls key lifecycle via erase().
    """
    ...
    def __init__(self, master_key: bytes, persist: bool = True):
        ...
        self._donor_keys: Dict[str, bytes] = {}  # donor_id → derived key
        self._erased: Dict[str, str] = {}         # donor_id → erasure timestamp
    ...
    def _record(self, action: AuditAction, donor_id: str,
    ...
    def _get_or_derive_key(self, donor_id: str) -> Optional[bytes]:
        """
        Get existing donor key or derive a new one.
        Returns None if donor has been erased.
        """
        if donor_id in self._erased:
            return None

        if donor_id not in self._donor_keys:
            key = _derive_donor_key(self._master_key, donor_id)
            self._donor_keys[donor_id] = key
            self._record(AuditAction.KEY_DERIVED, donor_id)
            if self._persist:
                self._save_keystore()

        return self._donor_keys[donor_id]

    # ── Public API ──────────────────────────────────

    def seal(self, donor_id: str, data: bytes) -> SealedBlob:
        """
        Encrypt donor data and return a sealed blob for append-only storage.
        ...
        Raises:
            PermissionError: If donor has been erased (cannot seal new data).
        """
        if donor_id in self._erased:
            raise PermissionError(
                f"Donor {donor_id} has been erased. Cannot seal new data. "
                f"Erasure is irreversible. (COV#015)"
            )

        key = self._get_or_derive_key(donor_id)
        nonce = os.urandom(12)

        # AAD includes donor_id for binding
        aad = f"kalaxi:donor:{donor_id}".encode("utf-8")
        ...
        self._record(
            AuditAction.DATA_SEALED, donor_id,
            detail=f"blob_id={blob_id}, content_hash={content_hash[:16]}..."
        )

        return blob

    def unseal(self, donor_id: str, blob: SealedBlob) -> Optional[bytes]:
        """
        Decrypt a sealed blob. Returns None if donor has been erased.

        Args:
            donor_id: Donor whose key should be used.
        ...
        if donor_id in self._erased:
            self._record(
                AuditAction.UNSEAL_DENIED, donor_id,
                detail=f"blob_id={blob.blob_id}, reason=donor_erased"
            )
            return None
        ...
        if plaintext is not None:
            self._record(
                AuditAction.DATA_UNSEALED, donor_id,
                detail=f"blob_id={blob.blob_id}"
            )

        return plaintext

    def erase(self, donor_id: str) -> ErasureResult:
        """
        Cryptographically erase a donor. Destroys the key material.
        The ciphertext remains in append-only storage but is unrecoverable.

        This is irreversible. There is no undo.

        Args:
            donor_id: Donor to erase.
        ...
        if donor_id in self._erased:
            return ErasureResult(
                ...
                timestamp=now,
                audit_entry_id="",
                message=f"Donor {donor_id} was already erased at {self._erased[donor_id]}.",
            )
        ...
        if donor_id not in self._donor_keys:
            return ErasureResult(
                ...
                timestamp=now,
                audit_entry_id="",
                message=f"Donor {donor_id} has no key material. Nothing to erase.",
            )

        # Destroy the key: overwrite with zeros then delete
        key_ref = self._donor_keys[donor_id]
        # Overwrite in-memory (best-effort; Python GC caveat acknowledged)
        zeroed = b'\x00' * len(key_ref)
        self._donor_keys[donor_id] = zeroed
        del self._donor_keys[donor_id]

        # Record erasure timestamp
        self._erased[donor_id] = now

        entry = self._record(
            AuditAction.KEY_ERASED, donor_id,
        ...
        return ErasureResult(
            ...
            message=(
                f"Donor {donor_id} erased. Key destroyed. "
                f"Ciphertext persists in append-only ledger (COV#003). "
                f"Data is cryptographically unrecoverable (GDPR Art. 17)."
            ),
        )

    def is_erased(self, donor_id: str) -> bool:
        """Check whether a donor's key has been destroyed."""
        return donor_id in self._erased

    def donor_status(self, donor_id: str) -> ErasureStatus:
        """Return the key lifecycle status for a donor."""
        if donor_id in self._erased:
            return ErasureStatus.ERASED
        if donor_id in self._donor_keys:
            return ErasureStatus.ACTIVE
        return ErasureStatus.NEVER_EXISTED

    def audit_trail(self, donor_id: Optional[str] = None) -> List[AuditEntry]:
        """
        Return the audit trail. Optionally filtered by donor_id.
        The audit trail is append-only and cannot be modified. (COV#010)
        """
        if donor_id is None:
            return list(self._audit)
        return [e for e in self._audit if e.donor_id == donor_id]

    def active_donor_count(self) -> int:
        """Count of donors with active (non-erased) keys."""
        return len(self._donor_keys)

    def erased_donor_count(self) -> int:
        """Count of donors whose keys have been destroyed."""
        return len(self._erased)

    # ── Persistence ─────────────────────────────────

    def _save_keystore(self) -> None:
        """Save keystore to disk. Keys are stored as hex. Erased donors recorded."""
        ERASURE_DIR.mkdir(parents=True, exist_ok=True)
        data = {
            "active_keys": {
                did: key.hex() for did, key in self._donor_keys.items()
            },
            "erased": self._erased,
            "counters": {
                "audit": self._counter,
    ...
    def _load_state(self) -> None:
        ...
        if KEY_STORE_PATH.exists():
            with open(KEY_STORE_PATH) as f:
                data = json.load(f)
            self._donor_keys = {
                did: bytes.fromhex(hexkey)
                for did, hexkey in data.get("active_keys", {}).items()
            }
...
def main():
    ...
    print()
    print("=" * 60)
    print("KALAXI Cryptographic Erasure — GAP#LEGAL-ERASURE-001")
    print("GDPR Article 17 <> COV#003 Append-Only Reconciliation")
    print("=" * 60)
    print()
    ...
    donor = "DONOR-042"
    secret = b"This is sensitive donor contribution data."

    # Seal
    print(f"[1] Sealing data for {donor}...")
    blob = engine.seal(donor, secret)
    ...
    # Unseal
    print(f"[2] Unsealing data for {donor}...")
    recovered = engine.unseal(donor, blob)
    assert recovered == secret, "Decryption failed!"
    print(f"    Recovered: {recovered.decode()}")
    print(f"    Status: {engine.donor_status(donor).value}")
    print()

    # Erase
    print(f"[3] Erasing {donor} (destroying key)...")
    result = engine.erase(donor)
    ...
    print(f"    is_erased: {engine.is_erased(donor)}")
    print()

    # Audit trail
    print(f"[5] Audit trail for {donor}:")
    for entry in engine.audit_trail(donor):
        print(f"    {entry.entry_id} | {entry.action:20s} | {entry.timestamp}")
    print()

    # Summary
    print(f"Active donors: {engine.active_donor_count()}")
    print(f"Erased donors: {engine.erased_donor_count()}")
    print()
