...
Version: 1.0
Grounded in: FOUNDATIONS/decay_function.md (Structural Proposal)
Linked Covenants: COV#004 (append-only), COV#001 (dignity-first)

Prevents Symbolic Sclerosis — the state where historical weight
drowns out the signal of the present donor.

Rules:
  1. No deletion — patterns recede, they do not vanish (COV#004)
...
class DecayEngine:
    ...
    def invoke(self, pattern_id: str) -> Optional[DecayRecord]:
        ...
        "Any donor invocation or link restores full weight."
        """
    ...
    def contest(self, pattern_id: str) -> Optional[DecayRecord]:
        ...
        "If a donor contests a pattern, its weight resets and it enters active review."
        """
        record = self._patterns.get(pattern_id)
        if record is None:
            return None
