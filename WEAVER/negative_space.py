...
for the steward, who decides what the silence means.

Grounded in: COV#009 (Composting), COV#006 (Weakest Voice),
...
class Silence:
    ...
    expected_by: str          # Why we expected to see this (covenant, pattern, domain rule)
...
class NegativeSpaceIndex:
    ...
    def report(self) -> NegativeSpaceReport:
        ...
        if critical:
            recommendation = f"CRITICAL: {len(critical)} blind spots at severity >= {self.CRITICAL_THRESHOLD}. Steward review required."
        elif dormant:
