...
to the donor's context — their domain, their history, their
current emotional state — without changing the proverb itself.

The proverb stays. The frame changes.

Grounded in: COV#001, COV#006, T#43 (Bone Flute — art as lamp),
...
class PersonalizedParables:
    """
    Adapts proverb delivery to donor context without altering the proverb.
    ...
    Usage:
        ...
        pp.register_donor("D-001", domains=["technology", "ethics"])
        delivery = pp.deliver("P#EMERGE-0022", "Capture fast. Tend slow.",
                              "D-001", donor_theme="velocity")
    """
    ...
    def __init__(self):
        self._donors: Dict[str, DonorContext] = {}
        self._deliveries: List[ParableDelivery] = []
        self._delivery_counter = 0

    def register_donor(self, donor_id: str, domains: Optional[List[str]] = None,
                       preferred_style: str = "direct") -> DonorContext:
        """Register a donor's context for personalization."""
        ctx = DonorContext(
            donor_id=donor_id,
            domains=domains or [],
            preferred_style=preferred_style,
        )
        self._donors[donor_id] = ctx
        return ctx

    def update_context(self, donor_id: str, domain: Optional[str] = None,
                       theme: Optional[str] = None) -> None:
        """Update donor context after an interaction."""
        ctx = self._donors.get(donor_id)
        if ctx is None:
            return
        ctx.history_count += 1
    ...
    def deliver(self, proverb_id: str, proverb_text: str,
                donor_id: str, donor_theme: str = "") -> ParableDelivery:
        """Deliver a proverb with personalized framing."""
        self._delivery_counter += 1
        ctx = self._donors.get(donor_id)

        # Build frame based on donor context
        style = ctx.preferred_style if ctx else "direct"
        domain = donor_theme or (ctx.domains[0] if ctx and ctx.domains else "your work")
        relevance = self._compute_relevance(proverb_text, ctx, donor_theme)
    ...
    def _compute_relevance(self, proverb_text: str,
                           ctx: Optional[DonorContext],
                           theme: str) -> float:
        """Compute how relevant this proverb is to the donor's context."""
        if ctx is None:
    ...
    def deliveries_for_donor(self, donor_id: str) -> List[ParableDelivery]:
        """Get all deliveries for a specific donor."""
        return [d for d in self._deliveries if d.donor_id == donor_id]
    ...
    def report(self) -> dict:
        ...
        return {
            "donors_registered": self.donors_count,
