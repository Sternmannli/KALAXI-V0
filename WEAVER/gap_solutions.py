...
Version: 1.0
Grounded in: COV#001 (dignity-first), COV#002 (silence is not closure),
             COV#008 (shelter path), COV#009 (testability)
...
Five gap solutions in one module:
  ...
  GAP#023 — Steward Shadow (Istihsan override delay)
    Any steward override of system wisdom must have thermal delay.
  ...
  GAP#024 — Shelter Heartbeat
    ...
    Silence after sheltering is abandonment (COV#002).
  ...
  GAP#026 — Contestability Protocol (appeal below Sealed Gate)
...
# "A system that over-halts is a system that silences."
# Linked: COV#001 (dignity-first), COV#009 (testability)
# ═══════════════════════════════════════════════════
...
class OverprotectionGuard:
    ...
    Three false-positive signals:
      ...
      3. Prior success: donor has exchanged before with similar language patterns
    ...
    def check(self, measurement, exchange_history: List[ExchangeRecord] = None) -> OverprotectionResult:
        ...
        Args:
            ...
            exchange_history: Prior exchanges from this donor for comparison.
        ...
        # ── Signal 3: Has donor exchanged successfully with similar language? ──
        prior_success = self._has_prior_success(exchange_history)
        if prior_success:
            false_positive_score += 0.3
            reasons.append("donor has prior successful exchanges — pattern may be benign")
        ...
        if false_positive_score >= 0.7:
            ...
            reason = "Moderate false-positive signal — steward should review. " + "; ".join(reasons)
        else:
    ...
    def _has_prior_success(self, exchange_history: List[ExchangeRecord]) -> bool:
        """Check if donor has prior successful exchanges."""
...
# GAP#023 — STEWARD SHADOW (Istihsan Override Delay)
# "Power exercised too quickly is power exercised without thought."
# Linked: COV#001 (dignity-first), COV#005 (Istihsan)
# ═══════════════════════════════════════════════════

class OverrideScope(Enum):
    """Scope of an Istihsan override."""
    MINOR = "minor"                    # Cosmetic, single-exchange
    MODERATE = "moderate"              # Affects covenant interpretation
    MAJOR = "major"                    # Affects Sealed Gate logic
...
class OverrideStatus(Enum):
    ...
    REJECTED = "rejected"              # Second steward rejected (major only)
...
class OverrideProposal:
    """
    A proposed Istihsan (steward override of system wisdom).
    ...
    scope: OverrideScope
    justification: str
    proposed_by: str                   # Steward identifier
    proposed_at: str
...
class StewardShadow:
    ...
    Delay schedule:
      - Minor overrides: 24h
      - Moderate (covenant interpretation): 7 days
      - Major (Sealed Gate logic): 30 days + second steward required
    ...
    def propose_override(
        ...
        Args:
            ...
            steward_id: Identifier of the proposing steward.
    ...
    def ratify_override(
        ...
        # Major scope requires second steward
...
# GAP#024 — SHELTER HEARTBEAT
# "Silence after sheltering is abandonment." (COV#002)
# Linked: COV#002 (silence is not closure), COV#008 (shelter path)
# ═══════════════════════════════════════════════════
...
class ShelterHeartbeat:
    ...
    Cadence:
      ...
      - After 3 pulses: system offers donor explicit exit option

    Donor can query shelter status at any time.
    ...
    def status(self, exchange_id: str, now: Optional[datetime] = None) -> Optional[dict]:
        """
        Donor-facing status query. Available at any time.
    ...
    def _build_message(
        ...
        """Build heartbeat message for steward notification."""
        prefix = {
...
# "Dignity is necessary but not sufficient for safety."
# Linked: COV#001 (dignity-first), COV#003 (safety), COV#009 (testability)
# ═══════════════════════════════════════════════════
...
class HarmDetector:
    ...
    def scan(self, text: str) -> HarmSignal:
        ...
        if severity >= 0.9:
            ...
            action = "steward_notification — include in next steward digest"
        else:
...
# "A system without appeal is a system without accountability."
# Linked: COV#001 (dignity-first), COV#002 (silence is not closure),
#         COV#009 (testability)
# ═══════════════════════════════════════════════════
...
class Contest:
    """
    A donor contest of a dignity halt.
...
class ContestabilityEngine:
    ...
    Rules:
      ...
      - Contest triggers steward review within 48h
      - If unreviewed after 48h, auto-escalate
      - Donor can provide additional context at any time before resolution
    ...
    def contest(self, exchange_id: str, reason: str) -> Contest:
        ...
        Args:
            ...
            reason: Why the donor believes the halt was incorrect.

        Returns:
            Contest record.

        Raises:
            ValueError: If reason is empty (COV#002: silence is not closure).
        """
        if not reason.strip():
            raise ValueError("Contest requires a reason — silence is not closure (COV#002)")
    ...
    def add_context(self, contest_id: str, context: str) -> bool:
        """
        Donor adds additional context to a pending contest.
    ...
    def begin_review(self, contest_id: str) -> bool:
        """Mark a contest as under steward review."""
        contest = self._ledger.get(contest_id)
        if contest is None:
            return False
    ...
    def resolve(
        ...
        Args:
            ...
            steward_note: Required explanation (COV#002).
        ...
        if not steward_note.strip():
            raise ValueError("Resolution requires a steward note (COV#002)")
