#!/usr/bin/env python3
"""
ed25519_sign.py — KALAXI Ed25519 Steward Signing CLI

Real cryptographic signing for steward decisions, canonical manifests,
and governance artifacts. Replaces HMAC-SHA256 stubs.
...
Commands:
  ...
  add     — Add a steward signature to an already-signed manifest
  verify  — Verify all signatures on a signed manifest
  multisig-check — Verify N-of-M threshold is met

Usage:
  python ed25519_sign.py gen --out-dir keys/steward1
  python ed25519_sign.py sign --key keys/steward1/private.hex --in manifest.json --out signed.json
  python ed25519_sign.py add --key keys/steward2/private.hex --in signed.json --out signed_2.json
...
def add_signature(existing_path: str, key_path: str, out_path: str):
    """Add another steward signature to an existing signed manifest."""
    _require_nacl()
...
def main():
    parser = argparse.ArgumentParser(
        description="KALAXI Ed25519 Steward Signing CLI",
    ...
    p_gen.add_argument("--out-dir", default="keys/", help="Directory to write keys (default: keys/)")

    # sign
    p_sign = sub.add_parser("sign", help="Sign a JSON manifest")
    p_sign.add_argument("--key", required=True, help="Path to private.hex")
    p_sign.add_argument("--in", required=True, dest="infile", help="Input manifest JSON")
    p_sign.add_argument("--out", required=True, dest="outfile", help="Output signed manifest JSON")

    # add
    p_add = sub.add_parser("add", help="Add steward signature to existing signed manifest")
    p_add.add_argument("--key", required=True, help="Path to private.hex")
    p_add.add_argument("--in", required=True, dest="infile", help="Existing signed manifest JSON")
    p_add.add_argument("--out", required=True, dest="outfile", help="Output signed manifest JSON")

    # verify
    p_verify = sub.add_parser("verify", help="Verify all signatures")
    p_verify.add_argument("--in", required=True, dest="infile", help="Signed manifest JSON")

    # multisig-check
    p_multi = sub.add_parser("multisig-check", help="Check N-of-M multi-sig threshold")
    p_multi.add_argument("--in", required=True, dest="infile", help="Signed manifest JSON")
    p_multi.add_argument("--threshold", required=True, type=int, help="Minimum valid signatures required")
    ...
    if args.cmd == "gen":
        ...
        ok = verify_signed(args.infile)
        sys.exit(0 if ok else 1)
    elif args.cmd == "multisig-check":
        ok = multisig_check(args.infile, args.threshold)
        sys.exit(0 if ok else 1)
    else:
        parser.print_help()
        sys.exit(1)
